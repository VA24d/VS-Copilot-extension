...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Usage dashboard}: requests by category, language, model, and day, in a per-user webview.
  \item \textbf{Cost tracking}: per-request Copilot cost units, monthly burn-rate forecast, daily spend pacing against a configurable budget.
  \item \textbf{Model-fit heuristic}: flags an expensive model used for routine work (or a lightweight one for something that needed more).
  \item \textbf{Estimated time savings}: directional per-category estimate.
  \item \textbf{Status bar indicator}: request count or remaining daily credit budget, amber warning once over pace, lock icon in private mode.
  \item \textbf{Company-wide reporting (opt-in)}: aggregate-only counts (category/language/model/day, never prompt/response text) shipped to an HTTPS endpoint or MongoDB on a schedule, plus a company-wide aggregate dashboard.
  \item \textbf{Private mode}: one toggle pauses all logging instantly; nothing is written to the local store.
  \item \textbf{Path exclusion \& sensitive redaction}: glob-based file exclusion and keyword-based redaction, applied before anything is written.
  \item \textbf{Configurable retention}: \code{retentionDays} auto-expires old rows.
  \item \textbf{CSV export}: \code{usageLogger.exportAuditCsv} for audit trails.
  \item \textbf{Knowledge grounding tools}: read-only Confluence, Jira, GitHub, SharePoint, and Teams search/lookup tools for Copilot chat.
  \item \textbf{\code{find\_help} skill}: routes non-code blockers (access, permissions, tooling, ``who do I ask'') to a contact/Teams chat/page/ServiceNow item from a local, org-curated directory --- fully offline.
  \item \textbf{Guided setup walkthrough}: in-editor walkthrough covering tokens, budgets, and the blocker directory.
\end{itemize}
...
\begin{sidewaysfigure}[p]
  \centering
  \includegraphics[width=\textheight]{media/architecture-diagram.png}
  ...
  knowledge-tool layer (rotated for readability at full page size).}
\end{sidewaysfigure}
