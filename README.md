...
- Status bar item shows today's request count (or a private-mode indicator); click it to open a dashboard (by category, language, model, and day), which auto-refreshes and shows the machine's hostname and the Lloyds Banking Group logo.
...
## Find help (who to contact) — the `#findHelp` skill

Not every blocker is a code problem. When someone needs **access, a permission, an environment fix, a tool, or just "who do I ask?"**, the `find_help` chat tool (`#findHelp`) matches the described blocker against a curated directory **you own** and points them at the sanctioned owner — a contact, a Microsoft Teams chat, an intranet/wiki page, and/or a ServiceNow catalogue item — instead of guessing or asking in the wrong channel.

It is **purely local and read-only**: no network calls, no credentials.

To make it yours:

1. Run **"Copilot Usage: Open Help Directory"** to start from the bundled sample (`media/blocker-directory.sample.json`).
2. Replace the placeholder contacts/links with your real owners.
...
Each entry is `{ title, keywords[], and optionally category, summary, contact, teamsLink, serviceNowUrl, url, steps[] }`. Only `https`/`http`/`msteams`/`mailto` links are surfaced; anything else is dropped defensively.
...
- **Copilot Usage: Open Help Directory** — open/scaffold the `#findHelp` blocker directory
...
- `usageLogger.blockerDirectoryPath` (default `""` = use bundled sample) — path to the `#findHelp` directory JSON
- `usageLogger.blockerDirectory` (default `[]`) — inline `#findHelp` directory entries
